...
function viewEmployee() {
  connection.query(
    "select id, first_name as firstName, last_name as lastName from employee",
    (err, employees) => {
      if (err) throw err;
  ...
  connection.query("select dept_name from department", (err, data) => {
    ...
    inquirer
      ...
      .then((response) => {
        ...
        connection.query(
          "select employee.first_name, employee.last_name, role.title, role.salary, department.dept_name from employee inner join role on employee.role_id = role.id inner join department on role.department_id = department.id where department.dept_name = ?",
          [chosenDepartment],
          (error, dataFinal) => {
            if (error) throw error;
            console.table(dataFinal);
  ...
  connection.query(
    ...
    (err, roles) => {
      ...
      connection.query(
        ...
        from employee`,
        (err2, emp) => {
          const currentEmployees = emp.map(
            ...
            .then(({ newFirstName, newLastName, newRole, newManager }) => {
              ...
              connection.query(
                ...
                (error, dataFinal) => {
                  if (error) throw error;
                  console.log(
                    `${newFirstName} ${newLastName} has been added to employees`
                  );
                  menu();
                }
              );
...
function addRole() {
  ...
  connection.query(
    "select id, dept_name as deptName from department",
    (err, depts) => {
      if (err) throw err;
      ...
      const departmentChoices = currentDepartments.map(
